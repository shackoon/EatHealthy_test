package com.example.admin.eathealthy.ColorArcProgressBar;
...
public class ColorArcProgressBar extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isNeedContent) {
            canvas.drawText(String.format("%.0f", curValues) + "/" + String.format("%.0f", maxValues), centerX, centerY + textSize / 3, vTextPaint);
        }
        if (isNeedUnit) {
            canvas.drawText(hintString, centerX, centerY + 4 * textSize / 3, hintPaint);
        }
        if (isNeedTitle) {
            canvas.drawText(titleString, centerX, centerY - 3 * textSize / 3, curSpeedPaint);
